...
export class BabylonChessView {
  ...
  setPositionFromFen(fen: string): void {
    ...
    for (let r = 0; r < 8; r++) {
      for (let f = 0; f < 8; f++) {
        ...
        mesh.position = this.squareToWorld(sq).add(new Vector3(0, 0.55, 0));
  ...
  async animateMove(from: string, to: string): Promise<void> {
    ...
    const end = this.squareToWorld(to as SquareKey).add(new Vector3(0, 0.55, 0));
  ...
  private createPieceMesh(type: string, color: "w" | "b"): Mesh {
    ...
    // You can later replace with GLB models.
    let mesh: Mesh;
